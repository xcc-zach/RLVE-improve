...
- 有效提示率
- 分布内accuracy
    生成训练的环境上从难度[0,4]均匀采样的100道题计算平均准确率
- 分布外accuracy
    从 #### 环境 涉及的训练环境以外的环境难度[0,4]均匀采样100道题固定为测试集，每次都在这个集合上计算平均准确率

#### 配置

- Adaptive Difficulty [h,h-1]
- Static Difficulty [0,1]
- Static Difficulty [0,4]
...
- 分布外accuracy
    在新增的几个环境（见新增环境设计）、难度[0,4]中均匀采样2500条样本，落盘作为该实验评测集。
